...
export default function SiteFooter() {
  return (
    <footer className="bg-ink px-4 text-stone-50 md:px-8">
      <div className="mx-auto grid max-w-7xl gap-8 py-10 md:grid-cols-[1fr_auto]">
        <div className="flex items-start gap-4">
          <span className="grid h-12 w-12 shrink-0 overflow-hidden rounded-full bg-ink">
          ...
          <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold text-stone-300">
            {cityLocations.map((city) => (
              <Link key={city.slug} href={cityPagePath(city)} className="rounded-full border border-white/10 px-3 py-1.5 transition duration-500 ease-luxury hover:border-accent/60 hover:text-white">
                {city.name}
              </Link>
            ))}
          </div>
      ...
      <div className="border-t border-white/10 py-5 text-center text-xs text-stone-300">
        Copyright 2026 GoodLuck Scrap. All rights reserved. · Designed by{" "}
        <a
          href="https://www.codesudio.com"
          target="_blank"
          rel="noopener noreferrer"
          className="transition duration-500 ease-luxury hover:text-white"
        >
          Codesudio
        </a>
      </div>
